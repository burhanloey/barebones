...
(defn choose-shade
  ...
  (let [position (Math/ceil (* (/ (Math/ceil count-data)
                                  highest-count)
...
(rf/reg-sub
 ...
 :<- [::year-data]
 (fn [year-data _]
   (find-details-for-year-data year-data)))

(rf/reg-sub
 ::last-7-days
 :<- [::year-data-details]
 (fn [year-data-details _]
   (take-last 7 year-data-details)))

(rf/reg-sub
 ::week-data
 :<- [::last-7-days]
 (fn [last-7-days _]
   (map :count last-7-days)))

(rf/reg-sub
 ::last-7-dates
 :<- [::last-7-days]
 (fn [last-7-days _]
   (map :date-obj last-7-days)))

(rf/reg-sub
 ::heatmap-data
 :<- [::year-data-details]
 (fn [year-data-details _]
   (generate-calendar-heatmap-data year-data-details)))
...
(rf/reg-sub
 ...
 :<- [::heatmap-data]
 (fn [heatmap-data _]
   (generate-months-legend-data heatmap-data)))
